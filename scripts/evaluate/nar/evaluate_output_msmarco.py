...
def compute_metrics(qids_to_relevant_passageids, qids_to_ranked_candidate_passages):
        ...
        if qid in qids_to_relevant_passageids:
            ...
            for i in range(0,MaxMRRRank):
                if candidate_pid[i] in target_pid:
                    ...
                    ranking.append(i+1)
                    break
            for i in range(min(6000, len(candidate_pid)-1)):
                if candidate_pid[i] in target_pid:
                    recall_num[i]+=1
    if len(ranking) == 0:
    ...
    all_scores['Recalll @300'] = sum(recall_num[:300])/len(qids_to_relevant_passageids)
    all_scores['Recalll @1000'] = sum(recall_num[:1000])/len(qids_to_relevant_passageids)
    all_scores['Recalll @2000'] = sum(recall_num[:2000])/len(qids_to_relevant_passageids)
    all_scores['Recalll @3000'] = sum(recall_num[:3000])/len(qids_to_relevant_passageids)
    all_scores['Recalll @6000'] = sum(recall_num[:6000])/len(qids_to_relevant_passageids)
